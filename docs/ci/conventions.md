...
### 2.1 원칙
- **SPOF 금지**: 핵심 경로는 개인 + 팀 중복 지정 (현재는 1인 개발 단계라 개인 지정).
- `.github/`, CI 워크플로, 패키지 메타데이터(`pyproject.toml`) 는 반드시 CODEOWNERS 커버.
...
### 3.1 워크플로 구조
- CI 워크플로는 **기능별로 분리된 파일**로 운영합니다 (가독성·관리성 향상).
...
- pip 캐시 활성화 (`actions/setup-python` 의 `cache: pip`).
...
- 예외: non-blocking 정보성 job (예: 커버리지 리포트 출력) 은 Required에 등록하지 않고
...
| 계정/App | 용도 | 허용 범위 |
|---------|------|-----------|
| `dependabot[bot]` | 의존성 자동 업데이트 | `pyproject.toml` / lock 파일 PR |
| _(추가 시 PR로 이 표 갱신)_ | - | - |
