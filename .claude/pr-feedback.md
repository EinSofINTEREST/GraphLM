...
상태 파일: `/tmp/graphlm-pr-feedback-loop.json`
...
4. 루프 상태 파일만 삭제: `rm -f /tmp/graphlm-pr-feedback-loop.json` (pr-watch-state.json 은 유지 — 재시작 시 기존 리뷰 PR 재감지 방지)
5. 사용자에게 알림: `"신규 PR 없음 20회 연속 (약 1시간) — pr-feedback loop 자동 종료 (cron <id>)"`

### 4. 상태 파일 갱신

매 회차 마지막에 `/tmp/graphlm-pr-feedback-loop.json` 을 현재 `idle_streak` 와 `last_run_at` 으로 갱신한다.
