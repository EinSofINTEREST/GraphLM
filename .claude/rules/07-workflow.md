...
해소하고, AI 의 자율성과 안전성의 균형을 명문화합니다.
...
- ad-hoc 으로 PR 직전에 이슈 만드는 패턴 차단 (closing reference 가 필요해서 PR 직전 즉흥 sub-issue 생성하는 안티 패턴)
...
- 의존성 추가 (`pip install` / `pyproject.toml` 갱신) — 단 신규 외부 패키지는 규약 5 적용
...
- **lint / fmt / test** — `make` 또는 `scripts/lint.sh` (ruff + nbqa) 실행, 경로 확인 불필요
...
```bash
git log --oneline (가상의 feature/#15 PR commit 시퀀스)
a1b2c3d [DOCS]: GraphEncoder docstring 추가 (en+ko)
e4f5g6h [FEAT]: GCN baseline forward 단위 테스트 추가
i7j8k9l [FEAT]: GraphEncoder 구현 — in/hidden/out dim 설정
m0n1o2p [REFAC]: TrainConfig 를 frozen dataclass 로 변경
q3r4s5t [CHORE]: pyproject.toml ruff 규칙 ICN001 추가
```
...
[CLAUDE.md](../../CLAUDE.md#pr-생성-후-자동-동작) 의 자동 동작 — `@.claude/loop.md`
...
- 동등 효과를 낼 수 있는 기존 허용 도구가 있으면 그것을 우선 사용 — 새 외부 도구 설치 X,
  기존 `gh` / `python` / `git` / `make` / `ruff` 등 활용
- `WebFetch` / `WebSearch` 도 새 도메인은 작업 명시적 필요 시에만
- 신규 외부 의존성 (Python package / system package) 추가는 **규약 2 의 "모호 영역"** 으로 간주
  → 사용자 사전 확인
...
| PR title | `[FEAT#N]` / `[FIX#N]` / `[REFAC#N]` / `[DOCS#N]` / `[CHORE#N]` (commit 카테고리 + #이슈번호, 콜론 없음) | `[DOCS#15] docstring 정정` | GitHub Actions PR Title Lint |
| **Issue title** | **`[FEATURE]` / `[FIX]` / `[REFACTOR]` / `[DOCS]` / `[CHORE]` / `[HOTFIX]`** (full-word, 콜론 없음) | `[FEATURE] GCN baseline 추가` | (lint 미강제, 규약 6 운영) |

**핵심 차이**: 이슈 prefix 는 commit prefix 의 축약형이 아니라 **원본 단어** 를 그대로 쓴다. 또한 commit/PR 에는 없는 `[HOTFIX]` 가 이슈 prefix 에만 존재 — 배포·실험 중 긴급 대응이라는 별도 카테고리. 이 표기 차이는 의도된 설계이며, 본 규약 6 의 매핑 표는 **이슈 prefix → Label/Type** 매핑이다.
...
본 repo 의 Issue Type ID 는 `scripts/gh-meta.sh` 상수로 관리한다 (변경 시 스크립트 갱신 필요).
...
다른 issue prefix 예: `[FEATURE] 새 baseline 모델`, `[REFACTOR] data loader 분리`, `[HOTFIX] eval 시 dim mismatch`.
...
6. **작업 완료 → PR 자동 생성 + Label 부여** (규약 4 + 규약 6) — `Closes #<sub-issue>` 명시, 마지막 sub-issue PR 에서 메인 이슈도 close → cron 자동 등록
